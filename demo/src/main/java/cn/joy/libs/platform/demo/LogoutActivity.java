...
public class LogoutActivity extends Activity implements PlatformActionListener {
	...
	@OnClick(R.id.btn1)
	void loginWeichat() {
		new AuthBuilder().authTo(Auth.Target.Wechat).listener(this).deAuth();
	}

	@OnClick(R.id.btn2)
	void loginQQ() {
		new AuthBuilder().authTo(Auth.Target.QQ).listener(this).deAuth();
	}

	@OnClick(R.id.btn3)
	void loginSina() {
		new AuthBuilder().authTo(Auth.Target.Sina).listener(this).deAuth();
	}
